...
.controller('dashboardCtrl', ['$scope','$state','adminserv', 'MENU_ITEMS', 'Auth', 'firebase', '$firebaseObject', '$firebaseArray', function($scope, $state, adminserv, MENU_ITEMS, Auth, firebase, $firebaseObject, $firebaseArray) {
	...
	listEntries.$loaded().then(function(){
		...
		for (var entry of $scope.entries) {
		 	if (entry.anores>=0) {
		 		$scope.entriesByYear[entry.anores-1].push(entry)
		 	}
		}
		...
		var mydataDailySalesChart = {
            labels: ['R1', 'R2', 'R3', 'R4'],
            series: [
                [$scope.entriesByYear[0].length, $scope.entriesByYear[1].length, $scope.entriesByYear[2].length, $scope.entriesByYear[3].length]
            ]
        };
